...
async function buscarInspiracao(estilo) {
    ...
    resultadoApi.innerHTML = "<p>Gerando paleta de cores para o seu look...</p>";

    try {
        const resposta = await fetch("https://www.thecolorapi.com/random");
        ...
        resultadoApi.innerHTML = `
            <div style="margin-top: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: #fff; text-align: center;">
                <h3>✨ Inspiração de Cor para o estilo ${estilo}</h3>
                <p>Aposte nesta cor como destaque na sua composição hoje:</p>
                
                <!-- Caixinha colorida gerada pela API -->
                <div style="width: 100px; height: 100px; background-color: ${corHex}; margin: 15px auto; border-radius: 50%; border: 2px solid #ccc;"></div>
                
                <p style="font-weight: bold; font-size: 16px;">${nomeCor}</p>
                <p style="color: #666; font-size: 14px;">Código: ${corHex}</p>
            </div>
        `;

    } catch (erro) {
        resultadoApi.innerHTML = "<p>Não foi possível carregar a paleta de cores.</p>";
